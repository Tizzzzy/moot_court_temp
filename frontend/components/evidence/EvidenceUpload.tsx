...
export function EvidenceUpload({
  ...
  // Category is "ready" if at least one valid file exists
  const hasAtLeastOneValidFile = validFilesCount > 0;
  const allFilesValid = uploadedFiles.length > 0 && validFilesCount === uploadedFiles.length;

  useEffect(() => {
    if (onUpdate && evidenceIndex !== null) {
      // Status logic: "ready" if at least one valid file, "needs-improvement" if only invalid files, "none" if not analyzed
      const status = hasAtLeastOneValidFile ? "ready" : invalidFilesCount > 0 ? "needs-improvement" : "none";
      onUpdate(
        evidenceIndex,
  ...
  }, [uploadedFiles, hasAnalyzed, evidenceIndex, onUpdate, hasAtLeastOneValidFile, invalidFilesCount]);
    ...
    <div className="bg-white rounded-[14px] border border-[rgba(0,0,0,0.1)] overflow-hidden">
      ...
      <div className="flex flex-col gap-6 p-6">
              ...
              <p className="text-[20px] text-[#0a0a0a] tracking-[-0.3125px]">
                ...
                {uploadedFiles.length > 0 && (
                  <>
                    ...
                    <span className="font-medium"> file{uploadedFiles.length !== 1 ? "s" : ""} uploaded</span>
                  </>
                )}
              </p>
              {hasAtLeastOneValidFile && hasAnalyzed && (
                <div className="bg-[#dcfce7] border border-[#b9f8cf] h-[22px] px-[5px] py-px rounded-[8px] flex items-center gap-2">
                  ...
                  <p className="font-medium text-[12px] text-[#016630]">Ready to use</p>
                </div>
              )}
            </div>
            <button
            ...
            <div className="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
              <div className="bg-[#f8fafc] flex flex-col items-center gap-4 py-[60px]">
                ...
                >
                  ...
                  <span className="font-medium text-[14px] tracking-[-0.1504px]">Upload Files</span>
                </button>
              </div>
            </div>
          ) : (
            ...
            <div className="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
              <div className="bg-[#f8fafc] p-3 flex flex-col gap-3">
                      ...
                      .map((file) => {
                        ...
                        return (
                          <div
                            ...
                            className={`${
                              isValid ? "bg-[#dcfce7] border-[#b9f8cf]" : "bg-[#eff6ff] border-[#bedbff]"
                ...
                {uploadedFiles.map((file) => (
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[16px] font-bold text-[#0f172b] tracking-[-0.3125px]">
                        {file.name}
                      </span>
                      ...
                      {file.status === "valid" && hasAnalyzed && (
                        <div className="ml-2 bg-[#dcfce7] border border-[#b9f8cf] h-[22px] px-[5px] py-px rounded-[8px] flex items-center gap-2">
                          ...
                          <p className="font-medium text-[12px] text-[#016630] capitalize">Valid</p>
                        </div>
                      )}
                      {file.status === "invalid" && hasAnalyzed && (
                        <div className="ml-2 bg-[#fef9c2] border border-[#ac7f5e] h-[22px] px-[5px] py-px rounded-[8px] flex items-center gap-2">
                          ...
                          <p className="font-medium text-[12px] text-[#ac7f5e] capitalize">Invalid</p>
                        </div>
                      )}
                    </div>
                    <button
                ...
                >
                  ...
                  <span className="font-medium text-[14px] text-[#0a0a0a] tracking-[-0.1504px]">Add new</span>
                </button>
              </div>
            </div>
          )}
        ...
        <div className="w-full flex items-center justify-end gap-3">
          ...
          >
            <span className="font-medium text-[14px] text-[#0a0a0a] tracking-[-0.1504px]">Cancel</span>
          </button>
          <button
          ...
          >
            <span className="font-medium text-[14px] text-white tracking-[-0.1504px]">
